...
async def _gerar_audio_com_timing(texto: str, voz: str, audio_path: str, rate: str) -> list[dict]:
    """
    Gera o áudio E coleta os WordBoundary timestamps.
    Retorna lista de {word, start_ms, duration_ms}.
    """
    communicate = edge_tts.Communicate(texto, voz, boundary="WordBoundary", rate=rate)
    words = []
    audio_chunks = []
...
def buscar_videos_da_colecao(api_key: str, collection_id: str, max_paginas: int = 5) -> tuple[list[dict], str]:
    """
    ...
    Retorna (videos, motivo_erro) — motivo_erro fica vazio se tudo correu bem
    (mesmo que a lista de vídeos esteja vazia por a coleção não ter vídeos).
    """
    videos = []
    pagina = 1
    erro = ""
    while pagina <= max_paginas:
        try:
            ...
            if resp.status_code != 200:
                erro = f"A API do Pexels respondeu {resp.status_code}: {resp.text[:200]}"
                break
            data  = resp.json()
            ...
            erro = f"Falha de conexão com a API do Pexels: {e}"
            break
    return videos, erro
...
if gerar:
    ...
    try:
        ...
        videos_colecao, erro_colecao = buscar_videos_da_colecao(pexels_key, collection_id)

        if not videos_colecao:
            msg = f"⚠️ Não encontrei vídeos na coleção (ID interpretado: '{collection_id}')."
            if erro_colecao:
                msg += f" Detalhe: {erro_colecao}"
            else:
                msg += " A API respondeu normalmente, mas sem vídeos — confira se essa coleção tem vídeos salvos (não só fotos)."
            st.warning(msg)
